...
-- IMPORTANTE:
-- Este script reinicia las tablas principales.
-- Si tienes datos previos, se eliminarán al ejecutarlo.
...
DROP FUNCTION IF EXISTS datetime(TIMESTAMP);

-- ============================================================================
-- FUNCIÓN AUXILIAR
-- Compatibilidad con authController.js:
...
RETURNS TIMESTAMP AS $$
  SELECT CASE
    WHEN lower(value) = 'now' THEN CURRENT_TIMESTAMP::timestamp
    ELSE value::timestamptz AT TIME ZONE 'UTC'
  END;
$$ LANGUAGE SQL STABLE;

CREATE OR REPLACE FUNCTION datetime(value TIMESTAMP)
RETURNS TIMESTAMP AS $$
  SELECT value;
$$ LANGUAGE SQL STABLE;
...
CREATE TABLE madres_cuidadores (
    ...
    correo_electronico VARCHAR(255) NOT NULL
        CONSTRAINT chk_correo CHECK (
            correo_electronico ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,20}$'
        ),
    ...
    zona_residencia VARCHAR(10) NOT NULL
        CONSTRAINT chk_residencia CHECK (
            zona_residencia IN ('Urbana', 'Rural')
        ),
    ...
    CONSTRAINT chk_consistencia_dos_hijos CHECK (
        (numero_hijos >= 2 AND tiene_dos_o_mas_hijos = TRUE)
        OR
        (numero_hijos < 2 AND tiene_dos_o_mas_hijos = FALSE)
    ),

    CONSTRAINT chk_consistencia_primeriza CHECK (
        (es_madre_primeriza = TRUE AND numero_hijos <= 1)
        OR
        (es_madre_primeriza = FALSE)
    )
);

CREATE INDEX idx_madres_correo_lower
ON madres_cuidadores (LOWER(correo_electronico));

CREATE INDEX idx_madres_telefono
ON madres_cuidadores (telefono);
...
CREATE TABLE recien_nacidos (
    ...
    sexo VARCHAR(15) NOT NULL
        CONSTRAINT chk_sexo CHECK (
            sexo IN ('Masculino', 'Femenino', 'M', 'F')
        ),
    ...
    CONSTRAINT chk_especificacion_complicaciones CHECK (
        ...
        (
            ...
            AND (
                especificacion_complicaciones IS NULL
...
CREATE INDEX idx_recien_nacidos_madre
ON recien_nacidos (madre_id);

-- ============================================================================
-- TABLA: evaluaciones_riesgo_registro
-- Evaluación inicial generada al finalizar el registro
-- ============================================================================
...
CREATE INDEX idx_eval_reg_madre
ON evaluaciones_riesgo_registro (madre_id);

CREATE INDEX idx_eval_reg_bebe
ON evaluaciones_riesgo_registro (bebe_id);

CREATE INDEX idx_eval_reg_fecha
ON evaluaciones_riesgo_registro (fecha_evaluacion);
...
CREATE TABLE evaluaciones_riesgo_bebe (
    ...
    nivel_riesgo VARCHAR(15) NOT NULL
        CONSTRAINT chk_nivel_riesgo CHECK (
            nivel_riesgo IN ('Bajo', 'Moderado', 'Alto')
        ),
    ...
    CONSTRAINT chk_consistencia_nivel CHECK (
        (
            ...
            AND (
                puntuacion_total >= 6
                OR convulsiones = TRUE
                OR dificultad_respiratoria = TRUE
                OR coloracion_azulada = TRUE
                OR fiebre_hipotermia = TRUE
                OR rechazo_alimentacion = TRUE
                OR disminucion_conciencia = TRUE
            )
        )
        OR
        (
...
CREATE INDEX idx_evaluaciones_riesgo_bebe_bebe
ON evaluaciones_riesgo_bebe (bebe_id);

CREATE INDEX idx_evaluaciones_riesgo_bebe_madre
ON evaluaciones_riesgo_bebe (madre_id);

CREATE INDEX idx_evaluaciones_riesgo_bebe_fecha
ON evaluaciones_riesgo_bebe (fecha_evaluacion);
...
CREATE TABLE seguimiento_diario_neonato (
    ...
    alimentacion_normal VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_alim_norm CHECK (
            alimentacion_normal IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    alimentacion_rechazo VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_alim_rech CHECK (
            alimentacion_rechazo IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    temperatura_fiebre VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_temp_fieb CHECK (
            temperatura_fiebre IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    temperatura_frio VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_temp_frio CHECK (
            temperatura_frio IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    actividad_normal VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_act_norm CHECK (
            actividad_normal IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    actividad_letargo VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_act_let CHECK (
            actividad_letargo IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    respiracion_normal VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_resp_norm CHECK (
            respiracion_normal IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    respiracion_dificultad VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_resp_dific CHECK (
            respiracion_dificultad IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    piel_normal VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_piel_norm CHECK (
            piel_normal IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    piel_alteracion VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_piel_alt CHECK (
            piel_alteracion IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    eliminacion_panales VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_elim_pan CHECK (
            eliminacion_panales IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    eliminacion_deposiciones VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_elim_dep CHECK (
            eliminacion_deposiciones IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    llanto_normal VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_llanto_norm CHECK (
            llanto_normal IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    llanto_alteracion VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_llanto_alt CHECK (
            llanto_alteracion IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    alarma_convulsiones VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_conv CHECK (
            alarma_convulsiones IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    alarma_vomito VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_vom CHECK (
            alarma_vomito IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    alarma_empeoramiento VARCHAR(15) NOT NULL
        CONSTRAINT chk_seg_empeoro CHECK (
            alarma_empeoramiento IN ('Mejoró', 'Igual', 'Empeoró', 'Sí', 'No')
        ),

    resultado_evolucion VARCHAR(10) NOT NULL
        CONSTRAINT chk_seg_resultado CHECK (
            resultado_evolucion IN ('Verde', 'Amarillo', 'Rojo')
        ),
...
CREATE INDEX idx_seguimiento_diario_bebe
ON seguimiento_diario_neonato (bebe_id);

CREATE INDEX idx_seguimiento_diario_madre
ON seguimiento_diario_neonato (madre_id);

CREATE INDEX idx_seguimiento_diario_triaje
ON seguimiento_diario_neonato (evaluacion_riesgo_id);
...
CREATE TABLE vacunacion_neonato (
    ...
    estado VARCHAR(20) NOT NULL DEFAULT 'Pendiente'
        CONSTRAINT chk_vac_estado CHECK (
            estado IN ('Pendiente', 'Aplicada', 'Atrasada')
        ),
...
CREATE INDEX idx_vacunacion_neonato_bebe
ON vacunacion_neonato (bebe_id);
...
CREATE TABLE controles_nino_sano (
    ...
    estado VARCHAR(20) NOT NULL DEFAULT 'Programado'
        CONSTRAINT chk_control_estado CHECK (
...
CREATE INDEX idx_controles_nino_sano_bebe
ON controles_nino_sano (bebe_id);

CREATE INDEX idx_controles_nino_sano_madre
ON controles_nino_sano (madre_id);
...
CREATE INDEX idx_bitacora_cuidado_bebe
ON bitacora_cuidado_bebe (bebe_id);

CREATE INDEX idx_bitacora_cuidado_madre
ON bitacora_cuidado_bebe (madre_id);
...
CREATE INDEX idx_bitacora_emocional_madre
ON bitacora_emocional (madre_id);

CREATE INDEX idx_bitacora_emocional_fecha
ON bitacora_emocional (fecha_registro);
...
CREATE TABLE evaluaciones_epds (
    ...
    clasificacion VARCHAR(20) NOT NULL
        CONSTRAINT chk_epds_clasificacion CHECK (
            clasificacion IN ('Bajo', 'Moderado', 'Alto')
        ),
...
CREATE INDEX idx_epds_madre
ON evaluaciones_epds (madre_id);

CREATE INDEX idx_epds_fecha
ON evaluaciones_epds (fecha_evaluacion);

-- ============================================================================
-- TABLA: notificaciones_alertas
-- IMPORTANTE:
-- leido es INTEGER porque el controller usa leido = 0 y leido = 1.
-- ============================================================================
...
CREATE TABLE notificaciones_alertas (
    ...
    leido INTEGER NOT NULL DEFAULT 0
        CONSTRAINT chk_notif_leido CHECK (leido IN (0, 1)),
...
CREATE INDEX idx_notificaciones_madre
ON notificaciones_alertas (madre_id);

CREATE INDEX idx_notificaciones_bebe
ON notificaciones_alertas (bebe_id);

CREATE INDEX idx_notificaciones_fecha
ON notificaciones_alertas (fecha_envio);
...
CREATE TABLE biblioteca_educativa (
    ...
    nivel_alerta VARCHAR(20)
        CONSTRAINT chk_biblioteca_nivel CHECK (
            nivel_alerta IS NULL
            OR nivel_alerta IN ('Bajo', 'Medio', 'Moderado', 'Alto', 'General')
        ),
...
CREATE INDEX idx_biblioteca_tema
ON biblioteca_educativa (tema);

CREATE INDEX idx_biblioteca_nivel
ON biblioteca_educativa (nivel_alerta);
...
CREATE INDEX idx_contacto_correo
ON contacto_mensajes (correo);

CREATE INDEX idx_contacto_fecha
ON contacto_mensajes (creado_en);
...
CREATE INDEX idx_testimonios_fecha
ON testimonios (creado_en);

-- ============================================================================
-- TABLA: password_reset_tokens
-- IMPORTANTE:
-- usado es INTEGER porque el controller usa usado = 0 y usado = 1.
-- expira_en es TEXT porque el controller compara con datetime(expira_en).
-- ============================================================================
...
CREATE INDEX idx_reset_madre
ON password_reset_tokens (madre_id);
...
SELECT 'Schema PostgreSQL de NeoCare ejecutado correctamente.' AS resultado;