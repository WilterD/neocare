...
const Register = () => {
  ...
  const clasificarPeso = () => {
    ...
    if (peso < 500 || peso > 7000) return "Peso no válido";
  ...
  const clasificarEdadGestacional = () => {
    ...
    if (semanas < 20 || semanas > 45) return "Edad gestacional no válida";
  ...
  const validarPasoActual = () => {
    if (step === 1) {
      ...
      const edadMadreVal = Number(edad);
      if (isNaN(edadMadreVal) || edadMadreVal < 12 || edadMadreVal > 60) {
      ...
      if (digitosTelefono.length > 7) {
        return "El teléfono debe tener como máximo 7 dígitos.";
      }
    ...
    if (step === 4) {
      ...
      const pesoVal = Number(pesoNacer);
      if (isNaN(pesoVal) || pesoVal < 500 || pesoVal > 7000) {
        return "El peso al nacer debe estar entre 500 y 7000 gramos.";
      }

      const semanasVal = Number(edadGestacional);
      if (isNaN(semanasVal) || semanasVal < 20 || semanasVal > 45) {
