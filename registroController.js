...
export const crearRegistro = async (req, res) => {
  try {
    ...
    // Parsear variables del bebé para validaciones previas
    const edadGestacional = Number(bebe.edadGestacional);
    const pesoNacer = Number(bebe.pesoNacer);

    // Validaciones lógicas
    if (isNaN(edadMadre) || edadMadre < 12 || edadMadre > 100) {
      return res.status(400).json({
        mensaje: "La edad de la madre debe estar entre 12 y 100 años."
      });
    }

    const digitosTelefono = telefonoMadre ? telefonoMadre.replace(/\D/g, "") : "";
    if (digitosTelefono.length > 7) {
      return res.status(400).json({
        mensaje: "El número de teléfono debe tener como máximo 7 dígitos."
      });
    }

    if (isNaN(edadGestacional) || edadGestacional < 20 || edadGestacional > 45) {
      return res.status(400).json({
        mensaje: "La edad gestacional debe estar entre 20 y 45 semanas."
      });
    }

    if (isNaN(pesoNacer) || pesoNacer < 500 || pesoNacer > 7000) {
      return res.status(400).json({
        mensaje: "El peso al nacer debe estar entre 500 y 7000 gramos."
      });
    }
    ...
    const result = await transaction(async (executeQuery) => {
      ...
      const pesoAlNacer = pesoNacer / 1000; // Convertir gramos a kg
      const edadGestacionalVal = edadGestacional;
      ...
      const resBebe = await executeQuery(sqlBebe, [
        madreId, nombreBebe, fechaNacimiento, pesoAlNacer, edadGestacionalVal, sexo,
